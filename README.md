...
生产服务会自动低频补全真实封面：默认启动 30 秒后开始，之后约每 90 秒从尚无真实封面的作品中选择一本，优先处理仙草数和评分较高的作品。阅读页仍会即时触发当前作品的补全，两种入口共用服务器端单并发队列。未匹配 30 天内不重试，普通错误 24 小时内不重试，遇到 403/429 会暂停整个队列 6 小时。生产环境的 `covers/` 因此需要对容器内的 `node` 用户可写。
