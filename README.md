...
阅读器还提供真实封面懒补全：作品只有 SVG 文字封面时，第一次进入阅读页会把任务放入服务器端单并发队列。队列每本只查询一次，随机间隔 5~8 秒；未匹配 30 天内不重试，普通错误 24 小时内不重试，遇到 403/429 会暂停整个队列 6 小时。生产环境的 `covers/` 因此需要对容器内的 `node` 用户可写。
