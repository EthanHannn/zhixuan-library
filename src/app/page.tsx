        ...
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="mt-6 flex gap-4">
            <input
              ...
              className="flex-1 px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#2F5D50] dark:bg-gray-800 dark:border-gray-600"
            />
            <select
              value={sortBy}
              onChange={(e) => {
          ...
          <>
            <div className="grid gap-4">
              {books.map((book, index) => (
                <div
                  key={book.id}
                  className="bg-white/40 dark:bg-gray-800/40 backdrop-blur-sm p-6 rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-lg transition"
                >
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-12 text-center">
                      <span className="text-2xl font-bold text-[#888888] dark:text-gray-400">
                        {(page - 1) * 20 + index + 1}
                      </span>
                    </div>
                    <div className="flex-1">
                      <Link href={`/book/${book.id}`}>
                        <h3 className="text-xl font-bold text-[#1a1a1a] dark:text-[#E8E4D9] hover:text-[#2F5D50] dark:hover:text-[#5A9A85] transition">
                          {book.title}
                        </h3>
                      </Link>
                      ...
                      <div className="flex gap-4 mt-3 text-sm">
                        <span className="text-[#2F5D50] dark:text-[#5A9A85]">
                          ⭐ {book.score.toFixed(2)}分
                        </span>
                        <span className="text-green-600 dark:text-green-400">
                          🌟 仙草 {book.xiancaoCount}
                        </span>
                        <span className="text-red-600 dark:text-red-400">
                          ☠️ 毒草 {book.ducaoCount}
                        </span>
