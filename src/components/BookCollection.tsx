...
export function BookCollection({ title, eyebrow = "CURATED COLLECTION", description, author, tag }: BookCollectionProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16">
      ...
      {description && <p className="mt-4 max-w-2xl leading-7 text-stone-500">{description}</p>}

      {loading ? (
        <div className="py-24 text-center text-stone-400">正在翻找书架…</div>
      ) : books.length ? (
        <>
      ...
      ) : <div className="mt-10 rounded-2xl border border-dashed border-[#d8d0c0] p-16 text-center text-stone-400">这个书架暂时是空的</div>}
    </main>
  );
}
