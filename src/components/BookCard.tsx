...
export function BookCard({ book }: { book: BookSummary }) {
  return (
    <article className="group overflow-hidden rounded-[1.4rem] border border-[#d9cfbf] bg-[#fffdf8] shadow-[0_12px_35px_rgba(58,49,39,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_45px_rgba(58,49,39,0.13)]">
      <div className="flex gap-4 p-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col py-1">
          ...
          <Link href={`/author/${encodeURIComponent(book.author)}`} className="mt-2 w-fit text-sm font-medium text-[#655f56] transition hover:text-[#a14f37] hover:underline">{book.author}</Link>
          ...
          <div className="mt-auto flex items-end justify-between gap-2 pt-3 text-xs font-medium text-[#746d62]">
            <span className="leading-5">{book.chapterCount ? `${book.chapterCount.toLocaleString("zh-CN")} 章` : "章节待整理"}<br />{formatWordCount(book.wordCount, book.size)}</span>
            <Link href={book.hasContent ? `/read/${book.id}` : `/book/${book.id}`} className="shrink-0 font-semibold text-[#a14f37] transition group-hover:translate-x-0.5">{book.hasContent ? "开始阅读 →" : "查看详情 →"}</Link>
          </div>
        </div>
      </div>
    </article>
